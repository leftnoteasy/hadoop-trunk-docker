...
ARG hadoop_tarball
ARG hadoop_prefix_dir
ENV HADOOP_PREFIX_DIR $hadoop_prefix_dir

COPY $hadoop_tarball /root/hadoop.tar.gz
RUN tar zxf /root/hadoop.tar.gz
ADD hadoop-conf-dir /root/hadoop-conf-dir 
COPY run_*.sh /root/
RUN chmod +x /root/*.sh

RUN echo "export HADOOP_PREFIX=/root/hadoop-3.0.0-alpha2-SNAPSHOT" >> /root/.bashrc
RUN echo "export YARN_CONF_DIR=/root/hadoop-conf-dir" >> /root/.bashrc
RUN echo "export HDFS_CONF_DIR=/root/hadoop-conf-dir" >> /root/.bashrc
RUN echo "export HADOOP_CONF_DIR=/root/hadoop-conf-dir" >> /root/.bashrc
RUN echo "export PATH=$PATH:/root/hadoop-3.0.0-alpha2-SNAPSHOT/bin:/root/hadoop-3.0.0-alpha2-SNAPSHOT/sbin" >> /root/.bashrc
